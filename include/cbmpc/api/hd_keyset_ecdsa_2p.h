...
// Run the 2-party HD keyset initialization protocol.
//
...
// Derive per-path ECDSA-2PC key blobs from an HD keyset.
//
...
//
// Output:
// - `out_ecdsa_2p_key_blobs.size() == non_hardened_paths.size()`
// - Each element is an `ecdsa_2p` key blob compatible with `coinbase::api::ecdsa_2p::*`.
...
// Extract the compressed root ECDSA public key Q from a keyset blob.
