...
// parallel_data_transport_t demonstrates that the core `job_*` types do not depend
// on any particular transport implementation.
//
// It also demonstrates one way to implement a "parallel transport": multiplexing
// several logical messages (distinguished by `parallel_id`) into a single
// underlying transport message, allowing multiple protocol sessions to share a
// single transport.
class parallel_data_transport_t : public coinbase::api::data_transport_i {
 public:
  parallel_data_transport_t(std::shared_ptr<coinbase::api::data_transport_i> _data_transport_ptr,
                            int _parallel_count = 1)
      : data_transport_ptr(std::move(_data_transport_ptr)) {
  ...
  // data_transport_i overrides using parallel_id 0
 ...
 private:
  std::shared_ptr<coinbase::api::data_transport_i> data_transport_ptr;
  int parallel_count;
...
class job_parallel_mp_t : public job_mp_t {
 ...
 public:
  job_parallel_mp_t(party_idx_t index, std::vector<crypto::pname_t> pnames,
      ...
      : job_mp_t(index, std::move(pnames), std::move(_network_ptr)), parallel_id(_parallel_id) {}

  void set_network(party_idx_t party_idx, std::shared_ptr<parallel_data_transport_t> ptr) {
    set_transport(party_idx, std::move(ptr));
  }

  job_parallel_mp_t get_parallel_job(int /*parallel_count*/, parallel_id_t id) {
    return job_parallel_mp_t(party_index, names, std::static_pointer_cast<parallel_data_transport_t>(transport_ptr), id);
  }
...
class job_parallel_2p_t : public job_2p_t {
 public:
  job_parallel_2p_t(party_t party, crypto::pname_t pname1, crypto::pname_t pname2,
      ...
      : job_2p_t(party, std::move(pname1), std::move(pname2), std::move(ptr)), parallel_id(id) {}

  void set_network(party_t party, std::shared_ptr<parallel_data_transport_t> ptr) {
    set_transport(party_idx_t(party), std::move(ptr));
  }
